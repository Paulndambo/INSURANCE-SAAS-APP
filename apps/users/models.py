...
class Membership(AbstractBaseModel):
    ...
    def get_membership_configuration(self):
        membership_config = self.membershipconfigs.filter(
            beneficiary__isnull=True).values().first()
        if membership_config:
            return membership_config
        else:
            return None

    def raise_future_premium(self):
        try:
            latest_premium = self.membershipprems.order_by("-expected_date").first()

            if latest_premium.status == "future":
                print("This policy already has a future premium!")
            else:
...
class PolicyHolderRelative(AbstractBaseModel):
    ...
    use_type = models.CharField(
        max_length=128, choices=USE_TYPES, default="dependent")
