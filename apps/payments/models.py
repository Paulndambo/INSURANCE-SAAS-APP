...
from apps.constants.choice_constants import PAYMENT_PERIOD_CHOICES, PAYMENT_METHODS, PAYMENT_STATUS_CHOICES, ACCOUNT_TYPES
...
class PolicyPayment(AbstractBaseModel):
    ...
    premium = models.FloatField(validators=[MinValueValidator(limit_value=0), ])
    vat = models.FloatField(validators=[MinValueValidator(limit_value=0), ], null=True)
...
class DebitOrder(AbstractBaseModel):
    ...
    date = models.IntegerField(validators=[MinValueValidator(limit_value=1), MaxValueValidator(limit_value=31)], null=True)
